...
export const createDitherRenderer: RendererFactory = (canvas) => {
  ...
  return {
    ...
    getAsciiText(frame, settings) {
      ...
      const cellCount = standardWidth * standardHeight;
      const lumBuf = new Float32Array(cellCount);
      ...
      const out = new Uint8Array(cellCount);
      floydSteinbergDither(lumBuf, standardWidth, standardHeight, out);

      // Text output is glyph-based (block / space); palette doesn't apply.
      // `invert` still flips which dither state maps to a filled block.
      const buffer = new Array<string>(cellCount + standardHeight);
      let bi = 0;

      for (let y = 0; y < standardHeight; y++) {
        const rowStart = y * standardWidth;
        for (let x = 0; x < standardWidth; x++) {
          const on = settings.invert
            ? out[rowStart + x] === 0
            : out[rowStart + x] === 255;
          buffer[bi++] = on ? "\u2588" : " ";
        }
